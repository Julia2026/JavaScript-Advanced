...
window.addEventListener('load', function(){
  section = document.getElementById('slider'),
  next = document.getElementById('NextSilde'),
  back = document.getElementById('PrevSilde');
  
  
  next.addEventListener('click', function() {
  var current = +section.getAttribute('data-current');
  current++;
  if (current >= OurSliderImages.length) {
    current = 0;
  }
  section.setAttribute('data-current', current);
  section.innerHTML = OurSliderImages[current];
  });
 ...
 back.addEventListener('click', function() {
  ...
  current--;
  if (current < 0) {
    current = OurSliderImages.length - 1;
  }
  section.setAttribute('data-current', current);
  section.innerHTML = OurSliderImages[current];
  });
